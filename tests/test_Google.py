...
def init_google_docs_keyring(client, can_create=True, input_getter=input):
    credentials = SimpleCredential('foo', 'bar')
    return Google.DocsKeyring(
        credentials,
        'test_src',
        NullCrypter(),
        client=client,
...
@unittest.skipUnless(is_gdata_supported(), "Need Google Docs (gdata)")
...
class GoogleDocsKeyringInteractionTestCase(unittest.TestCase):
    ...
    def _init_listfeed(self):
        ...
        listfeed._entry = [mocks.MockDocumentListEntry(), mocks.MockDocumentListEntry()]
        return listfeed
    ...
    def test_handles_retrieve_failure(self):
        client = self._init_client()
        client._listfeed = self._init_listfeed()
        client._request_response = dict(status=400, reason='Data centre explosion')
        self._init_keyring(client)
    ...
    def test_handles_corrupt_retrieve(self):
        client = self._init_client()
        dummy_entries = dict(section1=dict(user1='pwd1'))
        client._request_response = dict(
            status=200, data='broken' + self._encode_data(dummy_entries)
        )
        client._listfeed = self._init_listfeed()
        self._init_keyring(client)
        self.assertRaises(errors.InitError, self.keyring.get_password, 'any', 'thing')
    ...
    def test_no_set_if_create_folder_fails_on_new_keyring(self):
        ...
        self._init_keyring(client)
        self.assertEqual(
            self.keyring.get_password('service-a', 'user-A'),
            None,
    ...
    def test_no_set_if_write_fails_on_existing_keyring(self):
        ...
        client._request_response = dict(
            status=200, data=self._encode_data(dummy_entries)
        )
        client._put_err = gdata.service.RequestError
        client._listfeed = self._init_listfeed()
        self._init_keyring(client)
        self.assertEqual(
            self.keyring.get_password('sectionB', 'user9'),
            'pwd9',
    ...
    def test_handles_write_conflict_on_different_service(self):
        ...
        client = self._init_client()
        dummy_entries = dict(
            sectionWriteConflictA=dict(userwriteConflictA='pwdwriteConflictA')
        )
        client._request_response = dict(
            status=200, data=self._encode_data(dummy_entries)
        )
        client._put_err = [
            (gdata.service.RequestError, {'status': '406', 'reason': 'Conflict'})
        ]
        client._listfeed = self._init_listfeed()
        self._init_keyring(client)
        self.assertEqual(
        ...
        self.assertEqual(
            1, client._put_count, 'Write not called after conflict resolution'
        )
    ...
    def test_handles_write_conflict_on_same_service_and_username(self):
        ...
        client = self._init_client()
        dummy_entries = dict(
            sectionWriteConflictB=dict(userwriteConflictB='pwdwriteConflictB')
        )
        client._request_response = dict(
            status=200, data=self._encode_data(dummy_entries)
        )
        client._put_err = (
            ...
            {'status': '406', 'reason': 'Conflict'},
        )
        client._listfeed = self._init_listfeed()
        self._init_keyring(client)
        self.assertEqual(
            self.keyring.get_password('sectionWriteConflictB', 'userwriteConflictB'),
            'pwdwriteConflictB',
            'Correct password should be set in existing keyring',
        )
        conflicting_dummy_entries = dict(
            sectionWriteConflictB=dict(userwriteConflictB='pwdwriteConflictC')
        )
        client._request_response = dict(
            status=200, data=self._encode_data(conflicting_dummy_entries)
        )
        self.assertRaises(
            errors.PasswordSetError,
            self.keyring.set_password,
            'sectionWriteConflictB',
            'userwriteConflictB',
            'new_pwd',
        )
    ...
    def test_handles_write_conflict_with_identical_change(self):
        ...
        client = self._init_client()
        dummy_entries = dict(
            sectionWriteConflictC=dict(userwriteConflictC='pwdwriteConflictC')
        )
        client._request_response = dict(
            status=200, data=self._encode_data(dummy_entries)
        )
        ...
        new_pwd = 'Not the same pwd'
        conflicting_dummy_entries = dict(
            sectionWriteConflictC=dict(userwriteConflictC=new_pwd)
        )
        client._request_response = dict(
            status=200, data=self._encode_data(conflicting_dummy_entries)
        )
    ...
    def test_handles_broken_google_put_when_non_owner_update_fails(self):
        ...
        client = self._init_client()
        dummy_entries = dict(sectionBrokenPut=dict(userBrokenPut='pwdBrokenPut'))
        client._request_response = dict(
            status=200, data=self._encode_data(dummy_entries)
        )
        client._put_err = [
            (
                ...
                {
                    'status': '400',
                    'body': 'Sorry, there was an error saving the '
        ...
        correct_read_entries = dict(sectionBrokenPut=dict(userBrokenPut='pwdBrokenPut'))
        client._request_response = dict(
    ...
    def test_handles_broken_google_put_when_non_owner_update(self):
        ...
        self._init_keyring(client)
        new_pwd = 'newPwdBrokenPut'
        correct_read_entries = dict(sectionBrokenPut=dict(userBrokenPut=new_pwd))
        client._request_response = dict(
            status=200, data=self._encode_data(correct_read_entries)
        )
